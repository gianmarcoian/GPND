...
def compute_jacobian_autograd(inputs, output):
    """
    :param inputs: Size (e.g. Depth X Width X Height)
    :param output: Latent Size
    :return: jacobian: Latent Size X Size
    """
    assert inputs.requires_grad

    num_classes = output.size(0)  # Modifica per riflettere la dimensione corretta
    ...
    for i in range(num_classes):
        ...
        grad_output.zero_()
        grad_output[i] = 1  # Modifica per riflettere la dimensione corretta
